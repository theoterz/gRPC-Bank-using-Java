...
public class DbCon 
{
    ...
    public synchronized boolean authenticateUser(String username2, String password2) throws SQLException
    {    
        ...
        while ( rs.next() ) {
            ...
            if(usrname.equals(username2) && pswrd.equals(password2)) return true;
        }
        return false;
    }

    public synchronized int getId(String username, String password) throws SQLException
    {
        statement = connection.createStatement();
        ResultSet rs = statement.executeQuery( "SELECT * FROM users;" );
        while ( rs.next() ) {
            ...
            usrname = rs.getString("username");
            pswrd  = rs.getString("password");
            if(usrname.equals(username) && pswrd.equals(password)) break;
        }
